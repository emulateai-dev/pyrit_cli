...
from pyrit_cli.redteam.red_teaming import attack_converter_config_from_keys
from pyrit_cli.redteam.targets import openai_chat_from_spec


def _tap_success(result: AttackResult) -> bool:
...
async def _run_tap_once(
    *,
    ...
    objective: str,
    attack_converter_config: Any | None,
) -> AttackResult:
    ...
    return await attack.execute_async(**kwargs)  # type: ignore[misc]
...
async def run_tap_attack_async(
    *,
    ...
    request_converter_keys: list[str] | None = None,
    response_converter_keys: list[str] | None = None,
    ...
    user_conv = attack_converter_config_from_keys(
        list(request_converter_keys or []),
        list(response_converter_keys or []),
    )

    result = await _run_tap_once(
        objective_target_spec=objective_target_spec,
        adversarial_target_spec=adversarial_target_spec,
        ...
        objective=objective,
        attack_converter_config=user_conv,
    )

    if not _tap_success(result) and converter_fallback_on_failure:
